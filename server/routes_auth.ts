...
export function registerAuthRoutes(app: Express, sqlite: Database) {
  ...
  app.get("/api/staff", requireRole("owner", "admin"), (req, res) => {
    const rows: any[] = sqlite.prepare("SELECT id, name, role, position, gmail_email, gmail_connected, gmail_connected_at, phone, is_active, last_login_at, permissions, avatar_initials, created_at FROM employees ORDER BY id").all();
    res.json(rows.map(r => ({
