...
// import { registerAssistantRoutes } from "./routes_assistant"; // DISABLED — pulled per Cody 2026-09-15, will re-enable later
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  // means any route added in the future is protected automatically.
  {
    ...
    const PUBLIC_API = [
      "/api/health",
      // Public runtime config — exposes the Google Maps browser key so the
      // dashboard map can load. Browser keys are safe to expose (locked by
...
// ── Completion Certificate (backend generation info) ──────────────────────────
  ...
  // registerAssistantRoutes(app, sqlite); // DISABLED — pulled per Cody 2026-09-15
